...
def test_ops_flows_only_manual_review_filters_tasks() -> None:
    client = TestClient(app)
    api_key = "ops-manual-review-queue"
    headers = {"X-API-Key": api_key}

    purchase = client.post(
        "/v1/billing/packages/purchase",
        json={"package_code": "indie_entry_150", "reference": "invoice-ops-manual-review"},
        headers=headers,
    )
    ...
    created = client.post(
        ...
        json={
            ...
            "context": {"logs": "needs manual review"},
            "sla_seconds": 120,
            "max_price_usd": 2.5,
        },
        headers=headers,
    ...
    queue = client.get(
        "/v1/ops/flows",
        params={"only_manual_review": "true"},
        headers=_admin_headers(api_key),
    )
    assert queue.status_code == 200
    ids = {t["id"] for t in queue.json()}
